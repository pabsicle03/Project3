...
let xReportChart = new Chart(ctx, {
    type: 'bar',
    data: { labels: [], datasets: [{ label: 'Value', data: [], backgroundColor: 'rgba(166,15,45,0.7)', borderColor: 'rgba(166,15,45,1)', borderWidth: 1 }] },
    options: {
        responsive: true,
...
document.getElementById('loadBtn').addEventListener('click', loadXReport);
...
async function loadXReport() {
    ...
    try {
        const res = await fetch(`/api/xreport?date=${date}&metric=${metric}`);
